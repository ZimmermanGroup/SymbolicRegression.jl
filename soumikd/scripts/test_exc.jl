...
function my_custom_objective(tree, dataset::Dataset{T,L}, options)::L where {T,L}
    ...
    if (occursin("x1", repr(tree)))
        ...
        weighted_lim_loss2 = sum(dataset.weights .* (vxc .- exc_val .- (rho .*dexc_dr ./ drho_dr)).^2)/(dataset.n * sum(dataset.weights))

        return prediction_loss + 10*lim_loss1 + 100*weighted_lim_loss2
...
idx = findfirst(item -> item < 1e-3, rho)[1]
